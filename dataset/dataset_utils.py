...
def get_dataloader(args, train_flag=True):
    ...
    if args.dataset == 'ImageNet':
        ...
        transform = get_transformer(
            ...
            cropsize=args.input_shape[1], crop_padding=4, hflip=True
        )
    
        if train_flag:
            dataset = torchvision.datasets.CIFAR100(root=args.data_path, train=True, download=True, transform=transform)
            shuffle = True
        else:
